...
def plan(warehouse, dropzone, todo):
    ...
    for i in range(len(todo)):
        ...
        for j in range(len(warehouse)):
            if target in warehouse[j]:
                t = warehouse[j].index(target)
                start = [j, t]
                break
        value = [[100 for row in range(len(warehouse[0]))] for col in range(len(warehouse))]
        change = True
        while change:
            change = False
            for x in range(len(warehouse)):
                for y in range(len(warehouse[0])):
                    if dropzone[0] == x and dropzone[1] == y:
                        if value[x][y]>0:
                            value[x][y] = 0
                            change = True
                    elif warehouse[x][y] == 0 or ( x== start[0] and y == start[1] ) :
                        for a in range(len(delta)):
                            x2 = x + delta[a][0]
                            y2 = y + delta[a][1]
                            if x2 >= 0 and x2 < len(warehouse) and y2 >= 0 and y2 < len(warehouse[0]) :
                                if abs(delta[a][0]) == 1 and abs(delta[a][1]) == 1:
                                    v2 = value[x2][y2] + 1.5
                                else:
                                    v2 = value[x2][y2] + 1
                                ...
                                if v2 < value[x][y]:
                                    change = True
                                    value[x][y] = v2
        warehouse[start[0]][start[1]] = 0
        
        # for m in value:
        #     print m
        cost = cost + 2 * value[start[0]][start[1]]
...
print plan(warehouse4, dropzone4, todo4)
