...
############## ONLY ADD / MODIFY CODE BELOW THIS LINE ####################
    ...
    def cte(self, radius):
        ...
        #   
        cte = 0
        if self.orientation > 0 and self.orientation < pi/2.0  or ( self.orientation > pi/2.0 and self.orientation < pi):
            cte = (self.x - radius) ** 2+ (self.y  -radius) **2 - radius **2
        elif self.orientation == 0:
            cte = self.y - radius * 2
        elif self.orientation == pi:
            cte = self.y
        elif self.orientation > pi :
            cte =  (self.x - 3*radius) ** 2+ (self.y  -radius) **2 - radius **2
