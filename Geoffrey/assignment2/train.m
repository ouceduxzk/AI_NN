...
[numwords, batchsize, numbatches] = size(train_input);   % numwords = 3
vocab_size = size(vocab, 2);

% INITIALIZE WEIGHTS AND BIASES.
word_embedding_weights = init_wt * randn(vocab_size, numhid1); % vobab by hid1 matrix
...
for epoch = 1:epochs
  ...
  for m = 1:numbatches
    ...
    % why it should be a sparse matrix
    % so the sparse matrix 0(249 ge 0)1 represent the label of first word 

    expanded_target_batch = expansion_matrix(:, target_batch); % truncate the eye matrix to only first 100 colomn.
    ...
    % this derivative is the error * W_h * h*(1-h), the error term is the ∂CE/∂Output
    % the W_h is the ∂O/∂hidden , which is linear basis 
    % where h * (1-h) * W_e  is the derivative of h over embedding  , where is it sigmoid funciton.
    % but the W_e did not show here, it shows in the line 133, where back_propagated_deriv_2 is calculated
   ...
   % hid_bias_gradient = zeros(numhid2, 1);
    ...
    for w = 1:numwords
       word_embedding_weights_gradient = word_embedding_weights_gradient + ...
         ...
         % the expansion_matrix(:, input_batch(w,:))  term is to combine  the corresponding sparse vectors
         % into a matrix, where the index of  1 indicates the corresponding vocab out of 250 words

         % this is very important to update the embedding weigth gradient.
    end
